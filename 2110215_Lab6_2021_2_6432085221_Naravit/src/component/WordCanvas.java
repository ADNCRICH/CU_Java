...
public class WordCanvas extends Canvas {
		...
		Thread animationThread = new Thread(() -> {
			...
			try {
				for (double i = 21; i > 0; i--) {
					...
					// updateCanvas(); // CAUTION: This method contains UI update, which can cause
					// an error if running
				...
				for (double i = 0; i < 21; i++) {
					letterScale[index] = i / 20;
					 ...
					 // CAUTION: This method contains UI update, which can cause an error if running
									// in a different thread
					Platform.runLater(new Runnable() {
						@Override
						public void run() {
	...
	public void flipWord(Status[] newStatus) {
		 ...
		 * Please turn this into a separate thread instead
		 */
		Thread endTurnAnimationThread = new Thread(() -> {
			try {
				for (int i = 0; i < 5; i++) {
				...
				Thread.sleep(1000);
				// GameLogic.endTurns(); // CAUTION: This method contains UI update, which can
				// cause an error if running
				// in a different thread
				Platform.runLater(new Runnable() {
					@Override
					public void run() {
